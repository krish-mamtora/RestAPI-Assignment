...
@RestController
@RequestMapping("/api")
public class EmployeeMediaTypeVersioning {
    @Autowired
    VersionTestService versionTestService ;

    @GetMapping(value = "/employees" , produces = "application/vnd.employee-v1+json")
    public ResponseEntity<?> v1(){
        return ResponseEntity.ok(
                Map.of("version", "v1","data", versionTestService.getEmployeeV1())
        );
    }
    @GetMapping(value = "/employees" , produces = "application/vnd.employee-v2+json")
